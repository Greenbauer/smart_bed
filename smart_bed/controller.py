...
def scene_update():
    ...
    was_recently_occupied_left = (
        current_status['time_last_occupied_left'] is not None and
        not is_time_expired(
            current_status['time_last_occupied_left'], minutes=10)
    )

    was_recently_occupied_right = (
        current_status['time_last_occupied_right'] is not None and
        not is_time_expired(
            current_status['time_last_occupied_right'], minutes=10)
    )
    ...
    if is_fun_mode is not True:
        ...
        if (
            ...
            (
                was_recently_occupied_left is True or
